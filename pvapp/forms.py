...
from models import Member, db, Judge, Project, MentorPhoto, PastWinner
...
class NewMentorPhoto(Form):
  ...
  position = TextField("Position",[validators.Required("Please enter mentor position")])
  organization = TextField("Organization",[validators.Required("Please enter mentor organization")])
  bio = TextAreaField("Bio",[validators.Required("Please enter mentor bio")])
  photo = FileField('Photo', validators=[FileAllowed(['jpg','png','gif'], 'Please upload image file.')])
  def validate(self):
...
class AddMentorPhotoForm(Form):
  newmentorphoto = FormField(NewMentorPhoto)
  submit = SubmitField("Send")
  def validate(self):
    if not Form.validate(self):
      return False
    newmentorphoto = Member(self.newmentorphoto.data['name'], self.newmentorphoto.data['position'], self.newmentorphoto.data['organization'], self.newmentorphoto.data['bio'], session['mentorphoto'])
...
class NewJudge(Form):
  ...
  def validate(self):
    ...
    if checkmember or checkjudge: # check if email is already taken 
      ...
      return False
    judge = Judge(self.name.data, self.email.data.lower(), self.password.data)
    db.session.commit()
    return True
...
class NewSemifinalistScore(Form):
  presentationscore = SelectField('Presentation Score', coerce=int, choices=scorechoices, validators=[validators.Required("Please enter a Presentation Score for this submission")]) 
  innovationscore = SelectField('Innovation Score', coerce=int, choices=scorechoices, validators=[validators.Required("Please enter an Innovation Score for this submission")]) 
  planscore = SelectField('Implementation Plan Score', coerce=int, choices=scorechoices, validators=[validators.Required("Please enter an Implementation Plan Score for this submission")]) 
  comment = TextAreaField("Comments", description="Limit 2000 Characters")
  
class AddScoreForm(Form):
  newscore = FormField(NewScore)
  submit = SubmitField("Send")

class AddSemifinalistScoreForm(Form):
  newscore = FormField(NewSemifinalistScore)
  submit = SubmitField("Send")
...
class RoundTwoForm(Form):
  videolink = TextField('URL to Vimeo Upload', [validators.Required('Please enter the URL for your Video Pitch on Vimeo')])
  submit = SubmitField("Send")

class FinalRoundForm(Form):
  presentation = FileField('Presentation in PDF, PPTX, or PPT Format. Can include video formats M4V and MOV - must wrap everything in a ZIP file.', validators=[FileRequired(), FileAllowed(['pdf', 'pptx', 'ppt', 'zip'], 'Please only upload in one of the allowed formats.')]) 
  submit = SubmitField("Send")
